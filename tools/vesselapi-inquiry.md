...
~~**回答が届いたら本ファイルに追記すること。**~~
**【2026-09-19 追記】回答は 2026-09-04 に受領済み（下記「回答記録」）。実測は採用せず、AI 推計のみで運用する方針で決着している（下記「決定」、`tools/README.md`「採用は見送った」）。**
...
| 状態 | ~~回答待ち~~ → **2026-09-04 回答受領・決着**（規約上は許諾。カバレッジ不足のため実測は採用しない） |
...
> 【2026-09-19 注記】決定日の表記について：本節は「2026-09-04」、`tools/README.md` と上流の記録は「2026-09-05」としているが、
> どちらも同じコミット `12d68b1`（2026-09-05 00:40 JST）で書かれた**同一の決定**である。回答を受けた 9/4 の夜に判断し、日付をまたいで記録した。
